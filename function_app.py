...
@app.route(route="func_ytb_caption", methods=["POST", "OPTIONS"], auth_level=func.AuthLevel.ANONYMOUS)
def func_ytb_caption(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logging.warning(f"No transcript found for video {video_id}: {str(e)}")
        return func.HttpResponse(
            json.dumps({"error": f"No transcript available for this video in the requested language(s)."}),
            status_code=404, mimetype="application/json"
        )
    except VideoUnavailable as e:
        logging.warning(f"Video unavailable {video_id}: {str(e)}")
        return func.HttpResponse(
            json.dumps({"error": "Video is unavailable or private."}),
            status_code=404, mimetype="application/json"
        )
    except TranscriptsDisabled as e:
        logging.warning(f"Transcripts disabled for video {video_id}: {str(e)}")
        return func.HttpResponse(
            json.dumps({"error": "Transcripts are disabled for this video."}),
